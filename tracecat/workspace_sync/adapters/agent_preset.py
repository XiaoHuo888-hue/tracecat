...
class AgentPresetAdapter(DirectoryManifestAdapter):
    ...
    async def _version_specs_for_preset(
        ...
        *,
        ...
        mcp_meta_by_id = await self._mcp_integration_meta(
            workspace_service,
            {
                ref
        ...
        for version in version_rows:
            ...
            version_meta = {
                ref: mcp_meta_by_id[ref]
                for ref in mcp_integrations
                if ref in mcp_meta_by_id
            }
            versions[version.version] = AgentPresetVersionResourceSpec(
                ...
                mcp_integration_meta=version_meta or None,
    ...
    async def _mcp_integration_meta(
        self,
        workspace_service: SyncMappingService,
        refs: set[str],
    ) -> dict[str, McpIntegrationRefMeta]:
        """Return natural-key metadata keyed by local MCP integration id string."""
        ...
        return {
            str(row.id): McpIntegrationRefMeta(
                slug=row.slug,
